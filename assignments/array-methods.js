...
runners.forEach(function(x){
  return fullNames.push(`${x.first_name}, ${x.last_name}`)
})
...
runners.map(function(x){
  return firstNamesAllCaps.push(x.first_name.toUpperCase());
})
...
runners.filter( (x) =>{
  if(x.shirt_size === "L"){
    return runnersLargeSizeShirt.push(x);
  };
})
...
const AllEmails = []
runners.filter((x)=>{
  return AllEmails.push(x.email);
})
console.log(AllEmails)
// Problem 2
//filter for last name so we cant sort it alphabetically.
const InOrder = []
runners.filter(function(x){
  return InOrder.push(x.last_name);
})
console.log(InOrder.sort());

// Problem 3
// find company Toni is with.
const InOrder2 = []
runners.filter(function(x){
  if(x.first_name == "Toni"){
    return InOrder2.push(`${x.company_name}`);
  };
})
